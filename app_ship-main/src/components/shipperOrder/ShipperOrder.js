...
const ShipperOrder = () => {
    ...
    try {
      ...
      if (res && res.data) {
        ...
        if(res.data.isFeatured){
          try {
            const res = await PutDoneOrder(item.id,id666,"Done",true);
      
            if (res && res.data) {
              getOrders();
              Alert.alert('confirmed the successful delivery of the order');
            } else {
              Alert.alert('Error!');
            }
          } catch (error) {
  ...
  return (
    ...
    <Text style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 10, color: 'black' }}>Order received</Text>
            ...
            if (order.status === "Shipping") {
              ...
              return (
                <View key={order.id} style={styles.orderContainer}>
                  ...
                  <Button title="Delivery completed" onPress={() => handleImportOrders(order)} />
